...
def _build(mem:SharedMem):
    ...
    while not mem.file_queue.empty():
        ...
        for chunk, offset in ((num.first_digits[x:x+patterns_per_insert],x+1) for x in range(num.radix_pos-1, len(num.first_digits), patterns_per_insert)):
            patterns = get_patterns(chunk, size, offset)
            with mem.db_lock:
...
def progress(mem:SharedMem):
    ...
    while not mem.done:
        ...
        print(" "*70+"\r"+f"elapsed:{time_elapsed_f}\t eta:{eta}\t patterns:{speed_pattern_f}\t files:{mem.files_done}/{mem.files_total}", end="          \r")
...
def build(size:int, num_workers:int=6):
    ...
    """
    mem.files_total = 1
    mem.file_queue.put(get_one("pi",10,"txt"))
